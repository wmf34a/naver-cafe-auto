...
검수 신청은 하지 않는다. **앱을 등록한 계정 본인은 검수 없이 그냥 된다.**

### 앱은 각자 하나씩 만든다

여러 명이 쓸 때 **앱 하나를 공유하지 말고 각자 자기 계정으로 등록한다.** 등록자
본인은 자동 허용되므로 이 방식이 가장 단순하다.

- 인원 제한이 없다
- Client Secret 을 사람들에게 배포할 일이 없다
- 발행이 각자 계정 이름으로 나가서 후기 글쓴이가 제대로 남는다

앱 하나를 공유하려면 등록자 외의 아이디를 개발자센터 **내 애플리케이션 → 멤버관리**
탭에 등록해야 하고, **그 자리가 3개뿐이다.** 등록하지 않고 인증하면 "개발 중
상태에서는 등록된 아이디만 로그인할 수 있습니다"가 뜬다. 각자 만들면 이 제약이
아예 없어진다.

**단체 아이디로는 발행할 수 없다.** 네이버 단체 아이디는 네아로(네이버 아이디로
로그인)를 지원하지 않아서 OAuth 인증 자체가 막힌다. 개인 아이디를 쓴다.
...
| 첨부 이미지 | 본문 맨 끝에 몰리고, **발행 후 수정하면 깨진다**(엑박) |
| `<img>` + 외부 URL | 문단 사이 배치 가능하고, **발행 후 수정해도 안 깨진다** |
