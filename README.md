...
검수 신청은 하지 않는다. 검수 없이도 등록된 아이디로는 정상 동작한다.

**단체 아이디로는 발행할 수 없다.** 네이버 단체 아이디는 네아로(네이버 아이디로
로그인)를 지원하지 않아서 OAuth 인증 자체가 막힌다. 앱을 소유할 수는 있으니,
소유는 단체 아이디로 두고 실제 발행은 개인 아이디로 하는 구성을 권한다.

검수를 받지 않으면 **앱 등록자 본인 외에는 로그인할 수 없다.** 다른 사람이 쓰려면
개발자센터의 **내 애플리케이션 → 멤버관리** 탭에 그 사람의 네이버 아이디를 등록해야
한다. 등록하지 않고 인증하면 "개발 중 상태에서는 등록된 아이디만 로그인할 수
있습니다"가 뜬다.
